...
这轮研究给 `AgentMem` 最直接的结论不是“该选 vector-like 还是 filesystem-like”，而是：

> `AgentMem` 应先吸收 `vector/graph-like` 在 shared memory plane、temporal/entity modeling 上的优点，再以 `filesystem-like` 作为文件真相层、skill surface 和治理基座。
